...
(function (define) {
  define(['jquery'], function ($, root) {
    ...
    var GLYPH_CANCELAR = '<span class="glyphicon glyphicon-remove"></span>';
    var GLYPH_FLOPPY = '<span class="glyphicon glyphicon-floppy-disk"></span>'
    var LABEL = {
      Guardar: 'Cancelar',
      True: 'False',
      Si: 'No',
    ...
    var defaultSettings = {
      ...
      confirmLabel: Object.keys(LABEL)[3],
      guardarLabel: Object.keys(LABEL)[0],
      labels: LABEL,
    ...
    function _getFooter(buttons) {
      ...
      if (buttons) {
        ...
        messageFotter.append('<button class="x btn btn-default" data-dismiss=modal type=button><span class="glyphicon glyphicon-remove"></span>Cerrar</button>');
      }
    ...
    function _getModalInstance(skipEventChageIfExists) {
      ...
      function createModalElement() {
        return $('<div class="modal fade" tabindex="-1"><style>.modal-xl{width:96%;}.modal-body{max-height: calc(100vh - 145px);overflow-y: auto;}</style>' +
            ...
            ' <div class=modal-header><button type=button class="x close" data-dismiss=modal><span aria-hidden=true>&times;</span><span class=sr-only>Cerrar</span></button><h4 class=modal-title></h4></div>' +
            '</div>' +
            '</div>' +
            '</div>')
    ...
    function ajax(data, title) {
      ...
      function error(responseText, textStatus) {
        var msg = data.error ?
            ...
            '<strong>ERROR XHR: </strong>URL [ ' + params.url + '] load fail.' +
            '</div>';
    ...
    function confirm(data, title) {
      ...
      return alert({
        ...
        buttons: [{
          close: true,
          click: click,
          text: LABEL[data.label] ? LABEL[data.label] : GLYPH_CANCELAR+' '+LABEL[defaultSettings.guardarLabel],
          style: data.style && data.style[0] || 'default'
        }, {
          close: true,
          click: click,
          text: LABEL[data.label] ? data.label : GLYPH_FLOPPY+' '+defaultSettings.guardarLabel,
